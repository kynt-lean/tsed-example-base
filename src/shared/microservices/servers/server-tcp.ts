...
import { CLOSE_EVENT, EADDRINUSE, ECONNREFUSED, ERROR_EVENT, MESSAGE_EVENT, NO_MESSAGE_HANDLER, TCP_DEFAULT_HOST, TCP_DEFAULT_PORT } from '../constants';
import { TcpContext } from '../ctx-host';
import { Transport } from '../enums';
import { JsonSocket } from '../helpers';
import { CustomTransportStrategy, IncomingRequest, PacketId, ReadPacket, TcpOptions, WritePacket } from '../interfaces';
...
export class ServerTCP extends Server implements CustomTransportStrategy {
  ...
  public handleClose(): undefined | number | NodeJS.Timer {
    const retryAttempts = this.options ? parseInt(this.getOptionsProp(this.options, 'retryAttempts')?.toString() || "0") : 0;
    if (
      this.isExplicitlyTerminated ||
      !retryAttempts ||
      (retryAttempts && this.retryAttemptsCount >= retryAttempts)
    ) {
      return undefined;
    }
